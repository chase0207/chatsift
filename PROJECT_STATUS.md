...
### 进行中
- **W17 消息位置标识**(v0.4.0):阶段一本地层开发中(w17-message-position 分支,单 agent)
  - 身份+顺序统一靠会话内 position;有几条存几条(靠位置不靠内容);outbound 不存假时间;本地实时 + 云端异步对账;清空重采
  - 设计见 docs/prd/(W17 design/tasks)
