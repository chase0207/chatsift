...
async function authMiddleware(req, res, next) {
  ...
  try {
    ...
    // 旧 token 兼容：JWT 中无 is_super/data_scope/user_type/tenant_id，从数据库补充(W19)
    if (req.user.is_super == null || req.user.data_scope == null || req.user.user_type == null) {
      try {
        const [rows] = await pool.query(
          'SELECT r.is_super, r.data_scope, u.user_type, u.tenant_id FROM users u JOIN roles r ON r.id = u.role_id WHERE u.id = ? LIMIT 1',
          [req.user.id]
        );
        if (rows.length) {
