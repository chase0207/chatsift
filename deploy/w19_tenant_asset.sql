-- W19 租户资产模型 — 给已有库(prod/test v0.3.1)的手动迁移(不进 init)
-- 全新库由 server/sql/02_w19_tenant_asset.sql 基线随 init 建成,不跑本文件
-- 应用顺序:先 deploy/w17_message_position.sql(messages 结构),再本文件(依赖 platforms/platform_pages/users/业务表)
-- 性质:C 类。只增列/建表 + seed(幂等);ALTER ADD 非幂等,仅在已有库跑一次
-- 内容 = server/sql/02_w19_tenant_asset.sql(单一真源),如有更新以 server/sql/ 为准
