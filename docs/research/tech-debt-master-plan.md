...
- 🔄 **M1** W12.6 签收 → **由 W17 解决中**:W17 用 position 统一身份+顺序,从根上重做了消息时间/排序/去重,M1 的"时间准确率签收"并入 W17 阶段一验收。**W17 阶段一验收通过 = M1 关闭。**
- 🔄 **M13** 方向复核 → W17 清空重采时一并复核
- 🔄 **W17**(v0.4.0)进行中:Task1✓(数据契约)Task2✓(position算法)→ Task3-6
...
| 编号 | 项 | 严重度 | 现状 | 处置 | 三方对应 |
|---|---|---|---|---|---|
| M1 | W12.6 真实重采签收 → **W17 解决中** | 🟡 | W17 用 position 重做消息身份+顺序+时间,M1 并入 W17 阶段一验收 | **W17 阶段一验收通过 = M1 关闭**。原"95%抽样"由 W17 清空重采的真机验证替代 | cc-P1 / cc2-P1 |
