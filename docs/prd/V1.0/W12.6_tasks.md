...
| 版本 | 日期 | 变更摘要 | 触发来源 |
|---|---|---|---|
| v1.3.0 | 2026-06-01 | 修 codex 一直走兜底的真因:时间 p 标签不在消息内容节点*内部*,而在其祖先容器(向上2级的消息行的平级兄弟)。明确节点定位路径:从消息内容向上爬到 flex-col 容器,在容器内找 p.invisible | 用户 DOM 层级定位 |
| v1.2.0 | 2026-06-01 | 发现 invisible 精确时间标签,改为优先读 | 用户 DOM 审查 |
| v1.1.0 | 2026-05-31 | 相对时间解析 + 锚点继承 + 默认看最新 | 诊断 |
| v1.0.0 | 2026-05-31 | 初版诊断 | 真实页面验收 |
...
> **v1.3.0 关键修正:codex 一直走兜底,真因是节点定位错了。**
>
> 时间 p 标签**不在消息内容节点内部**,而在其**祖先容器里、是『消息行』的平级兄弟**。所以"在消息 div 内部找时间"必然找不到,每次 fallback。
>
> 真实 DOM 层级(用户定位):
> ```
> <div class="flex-1 flex flex-col">                    ← 消息列容器(时间和消息行都在这下面)
>   <p class="text-xs text-gray-3 absolute whitespace-nowrap invisible"
>      style="left:55px;top:-18px;">2026-06-01 14:26:36</p>   ← 精确时间(消息行的前置兄弟)
>   <div class="flex items-center">                     ← 消息行
>     <div class="...leftMsg-ewM7qC">
>       <div class="...whitespace-pre-wrap">谢谢你</div>  ← 消息内容
>     </div>
>   </div>
> </div>
> ```
> 即:从消息内容节点"谢谢你"**向上2级到消息行(flex items-center)**,时间 p 是它的**平级兄弟**;或再向上到 `flex-1 flex flex-col` 容器,在容器内找 p.invisible。

改 `plugin/adapters/douyin/private-message.adapter.js` 的时间提取,**定位路径要对**:

```
对每条消息(从消息内容节点出发):
  1. 向上爬到祖先容器:
     contentNode(.whitespace-pre-wrap)
       → 向上找最近的 .flex-1.flex-col 容器(消息列容器)
       (或:contentNode 向上2级到消息行 .flex.items-center,再取其 previousElementSibling)
  2. 在该容器内找时间 p:
     container.querySelector('p.invisible.whitespace-nowrap.absolute')
     或遍历容器直接子节点找 class 含 invisible+whitespace-nowrap 且内容匹配日期时间的 p
  3. 双重确认:内容正则匹配 /\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}/
  4. 读到 → 解析为 occurred_at(精确到秒)
  5. 读不到 → Fix Task B 兜底
```

**关键:别在消息内容节点的子树里找时间**(找不到)。要先向上到正确的祖先容器层,时间 p 和消息行是兄弟关系。

注意:
- 祖先定位用 closest('.flex-col') 之类向上找,比写死层数稳(DOM 可能有微调)
- 但 closest 依赖 class,若 flex-col 这种 class 不稳,用"向上 N 级 + 验证容器内有 invisible p"组合
- 每条消息独立精确时间,顺序天然正确

### Fix Task A-old(v1.2.0 原描述,定位不够明确,已被 v1.3.0 修正)
...
阶段二(v1.3.0):
- [ ] Fix Task A:**节点定位对**——从消息内容向上到祖先容器,在容器内找 invisible 时间 p(不在消息内部找)
- [ ] **真实时间准确率 ≥ 95%**:重采后抽样 N 条,对比入库 occurred_at 与 hover 显示的真实时间,一致率 ≥95%(即绝大多数走精确时间标签,而非兜底)
- [ ] 验证不再"每条都走兜底":查 raw_snapshot 的 time_source,绝大多数应是"精确标签"而非"anchor/estimated"
- [ ] Fix Task B 兜底:个别读不到的才走,不崩、不继承旧时间
- [ ] 入库 id 顺序与 DOM 一致;前端默认滚到最新
- [ ] 清旧数据重采:chase/车评老帅 时间精确、顺序对、"儿童节快乐"显示今天
...
- **codex 一直走兜底的真因是节点定位错**:时间 p 标签不在消息内容节点*内部*,而在祖先容器里、是消息行的平级兄弟。必须从消息内容**向上爬到祖先容器**(.flex-col),再在容器内找 invisible 时间 p。别在消息 div 子树里找(找不到→必然兜底)
- **目标:真实时间准确率 ≥95%**——绝大多数消息走精确时间标签,只有极少数兜底。验证查 raw_snapshot 的 time_source 分布
- **选择器双重确认**:class 含 invisible+whitespace-nowrap+absolute + 内容正则匹配 YYYY-MM-DD HH:mm:ss
- **祖先定位用 closest 向上找**比写死层数稳;class 不稳时用"向上N级+验证容器内有invisible p"
- **兜底时解析失败不继承旧时间**,用采集时刻+标记估算
- **默认看最新消息**:前端体验,无论如何都做
- **occurred_at 影响 intent(最近20条)/completeness(全量)**,修准后重跑回归
- **重采前先确认插件已重新加载**
- **C 类**(动 W4 采集),改完重跑采集验收
- **不要改 chat_rpa**(只读参考)
