...
## ★ E1 / E2 验收通过总结(2026-06-09,Chase 确认)

### E1 — dev 库 W20 落地 ✅ 通过
- 目标库 `chatsift`@127.0.0.1:3306(非 test/prod)清库重采(选项B)+ apply `deploy/w20_account_governance.sql`。
- post-check:W20 四列 + 三表建好、`uk_account` 去昵称、业务表清空、只读接口 10/10 HTTP 200。详见下方 E1 明细。

### E2 — 真机全链路回归 ✅ 通过
真机跑通(经多轮诊断修复后):
| 链路 | 结果 |
|---|---|
| 首见 → pending | 客服插件采未知账号 → 自动建 pending,谁发现给谁临时采集权(不区分角色)|
| 管理后台确认 | http://localhost:5173 超管 confirm pending → active + 指派正式采集人 |
| 采集权拦截 | 非采集负责人采集 → event 级 rejected(pending_grab/not_collector),不入库;HTTP 200 |
| 查看权隔离 | 查看人能看不能采 |
| disabled 冻结 | 停用 → 全员采集冻结;恢复可采 |
| 实例冲突仲裁 | 同客服两实例同账号 → **最早注册继续、更晚暂停**;关闭后恢复独占 |
| 状态可见 | 采集权拒/实例冲突写入插件 popup「消息日志」|
| admin 治理闭环 | 确认/重分配采集人/查看人增删/停用恢复,不依赖手写 SQL |
| 采集开关 | **修:`collector_v1_enabled` 默认 false,以"启动/停止"为开关**(commit 0362823)|

★关键修复(让 E2 通过):heartbeat β(只治同采集负责人多实例,80c0db7)→ 采集权口径"谁发现给谁"(99f4e7a)→ 冲突账号级(98a0b2a)→ 冲突仲裁最早为主(1b50fa3)→ 启停开关默认 false(0362823)。

### 未完成
E3 三角色×三态 SQL 可见性断言、E4 红线复核收口、merge/部署(待 Chase)。
