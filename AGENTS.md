...
> 解决"编号种类太多、字母复用、跨 agent 不统一"的混乱。持久编号口径统一归 `docs/ops/versioning.md`,本节只保留沟通临时引用和固定术语。

三种持久编号 W(迭代)/ M(技术债)/ v(版本)的定义见 `docs/ops/versioning.md` §1。
...
| 版本 | 日期 | 变更摘要 |
|---|---|---|
| v1.8.0 | 2026-06-14 | 编号约定 W/M/v 定义改为指向 versioning §1,保留 Q# 与 A/B/C 类固定术语 |
